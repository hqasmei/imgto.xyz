...
const WidgetUpload = ({ className }: WidgetUploadProps) => {
  ...
  const uploadingCount =
    imageStates?.filter((state) => state === "uploading").length || 0;
  const optimizingCount =
    imageStates?.filter((state) => state === "optimizing").length || 0;
  const errorCount =
    imageStates?.filter((state) => state === "error").length || 0;
  const finishedCount =
    (imageStates?.filter((state) => state === "finished").length || 0) +
    errorCount;
  const totalCount = Array.isArray(images) ? images.length : 0;

  const isUploading = typeof uploadingCount === "number" && uploadingCount > 0;
  const isOptimizing =
    typeof optimizingCount === "number" && optimizingCount > 0;
  ...
  const hasDownloadsAvailable =
    typeof finishedCount === "number" && finishedCount > 0;
  ...
  const totalSizeOriginal =
    images && addNumbers(images.map(({ size }) => size));
  const totalSizeOptimized =
    isUploadComplete &&
    images &&
    addNumbers(images.map(({ optimized }) => optimized?.size || 0));
            ...
            try {
              ...
              if (e instanceof Error) {
                if (e.message === "INAPPROPRIATE_CONTENT") {
                  message =
                    "An image you uploaded appears to be inappropriate and not supported by imgto.xyz. If you think this is in error, please email community@cloudinary.com.";
            ...
            try {
              ...
              if (e instanceof Error) {
                if (e.message === "UNAUTHORIZED") {
                  message =
                    "Someting went wrong. Try disabling any Ad Blockers and try again!";
                }
              }
  ...
  async function handleOnDownloadAll() {
    ...
    const downloads = images
      ...
      .map(({ name, upload, optimized }) => {
        return {
          name: fixFileName(name, upload?.format),
    ...
    await downloadUrl(
      `/api/archive?urls=${JSON.stringify(downloads)}`,
      "imgtoxyz.zip"
    );
  ...
  // Utility to fix the file name
  function fixFileName(name: string, format?: string) {
    const extension = format ? `.${format}` : "";
    // Remove the extension if already present and re-add it to prevent duplicates
        ...
        {Array.isArray(images) && (
          <section>
            <h2 className="flex justify-between mb-1">
              ...
              {totalSizeOriginal && totalSizeOptimized && (
                <span className="text-xl font-bold text-green-600">
                  ...
                  {(
                    100 -
                    (totalSizeOptimized / totalSizeOriginal) * 100
                  ).toFixed(0)}
                  %
                </span>
              )}
            ...
            <p className="text-sm mb-5">
              Total Original Size:{" "}
              {totalSizeOriginal &&
                formatBytes(totalSizeOriginal, { fixed: 0 })}
            </p>
            <ul
              className={cn(
                `grid gap-2 mb-7`,
            ...
            <div className="mb-6">
              ...
              <div className="flex justify-between">
                <p className="text-xs font-bold">{stateMap[globalState]}</p>
              ...
              <div className="flex justify-between mb-6">
                <div>
                  ...
                  <p className="block text-sm font-bold">
                    Total Optimized:{" "}
                    {totalSizeOptimized &&
                      formatBytes(totalSizeOptimized, { fixed: 0 })}
                  </p>
                </div>
              </div>
            )}
